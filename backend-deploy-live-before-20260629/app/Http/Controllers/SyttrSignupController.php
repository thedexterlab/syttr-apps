...
class SyttrSignupController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'Syttr signup successful',
        ...
        app()->terminating(function () use ($internalUserId): void {
            try {
                ...
                if (! ($ghlResult['success'] ?? false)) {
                    Log::warning('syttr_signup.ghl_sync_failed', [
                        'user_id' => $user->user_id,
                ...
                Log::warning('syttr_signup.deferred_ghl_sync_failed', [
    ...
    private function deferWelcomeEmail(int $internalUserId): void
    {
        app()->terminating(function () use ($internalUserId): void {
            try {
                ...
                Log::warning('syttr_signup.deferred_welcome_email_failed', [
                    'internal_user_id' => $internalUserId,
                    'message' => $e->getMessage(),
                ]);
            }
